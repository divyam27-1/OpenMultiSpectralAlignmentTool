...
public struct ProgressBar {
    ...
    public void update(int completed, int failed = 0) {
    ...
    private void render() {
        ...
        // [==========>----------] 50% (5/10)
        string bar = "[" ~ 
                     "=".repeat(max(0, filledWidth - 1)).join() ~ 
                     (filledWidth > 0 ? ">" : "") ~ 
                     "-".repeat(emptyWidth).join() ~ 
                     "X".repeat(failedWidth).join() ~
                     "]";

        // \r moves cursor to start of line. 
        // writef keeps us on the same line.
        writef("\r%s %3.0f%% (%d/%d)\t", bar, percentSucess * 100, completedCount, totalCount);
